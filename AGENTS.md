...
1. Read `DESIGN.md`, approved `deployment/assets/downloads/citychat-ds-addon-v0.7.md`, `deployment/assets/downloads/citychat-ds-addon-v0.7-approval.json`, and draft `deployment/assets/downloads/citychat-product-experience-profile-v0.4.md`.
...
14. In artifact v0.7.0, inherit LDS `.btn` and `.btn-icon` geometry and feedback. Do not replace their padding, radius, height, focus, disabled, busy, or press contract with a local button recipe.
15. Resolve functional icons through `deployment/resources/citychat-ds-addon/v0.7.0/citychat-icon-map.json` and the actual-control bindings in `citychat-icon-resolution.json`. Use the pinned local font file and its manifest; never use the CityChat lockup or motif as an action icon, and never infer capability from a library specimen.
16. Resolve product colour jobs through `citychat-color-role-map.json`. It is a CityChat usage overlay on the pinned LDS colour source, not a second palette or an analytical-scale registry. Missing authority fails closed to no-data or unavailable copy.
17. Resolve motion through `semantic-motion.citychat.yml`: an inherited LDS primitive or `none`. Keep first meaning and action visible, make no-JavaScript and reduced-motion final states readable, and never let animation create a receipt, product event, telemetry, or capability claim.
18. Artifact v0.7.0 implements approved normative DS Add-on v0.7. Do not cite the implementation records as DS Add-on v0.7.0 normative authority.
